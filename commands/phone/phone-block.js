	...
	async run(msg, { query }) {
		...
		if (user) {
			return msg.say(stripIndents`
				...
				Place \`<chillax:phone:block:${user.id}>\` in this channel's topic
			`);
		}
		...
		return msg.say(stripIndents`
			...
			Just the channel: Place \`<chillax:phone:block:${channel.id}>\` in this channel's topic
			The entire server: Place \`<chillax:phone:block:${channel.guild.id}>\` in this channel's topic
		`);
	}
};
